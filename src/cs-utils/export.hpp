...
void savePPM(std::vector<T> const& data, uint64_t width, uint64_t height, std::string_view fileName) {
  ...
  for (const auto& pixel : data) {
    oss << static_cast<uint16_t>(lround(std::clamp(pixel.r, 0.0, 1.0) * MAX_VALUE)) << " "
        << static_cast<uint16_t>(lround(std::clamp(pixel.g, 0.0, 1.0) * MAX_VALUE)) << " "
        << static_cast<uint16_t>(lround(std::clamp(pixel.b, 0.0, 1.0) * MAX_VALUE));
